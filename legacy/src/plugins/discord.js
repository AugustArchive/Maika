...
module.exports = new Plugin({
    ...
    commands: [{
        ...
        async run(bot, msg) {
            const channel = await bot.finder.channel(msg.args.length > 0 ? msg.args.join(' ') : msg.channel.id, msg.guild);
            let embed = {
            ...
            if (!msg.guild || !msg.guild.channels.has(channel.id))
                embed.fields.push({ name: 'Guild', value: bot.guilds.get(bot.channelGuildMap[channel.id]).name, inline: true });
    ...
    },
    {
        ...
        category: 'Discord Information',
        async run(bot, msg) {
            const emojis = await bot.finder.emojis(msg.guild);
            return msg.embed({
                description: emojis,
                color: bot.color
            });
    ...
    },
    {
        ...
        async run(bot, msg) {
            ...
            try {
                ...
                const message = await bot.finder.message({ channelID: args[0], messageID: args[1] });
    ...
    },
    {
        ...
        async run(bot, msg) {
            ...
            const role = await bot.finder.role(msg.args[0], msg.guild);
            return await msg.embed({
                title: `Role ${role.name}`,
                color: (role.color === 0 ? bot.color : role.color),
                fields: [
                    {
    ...
    },
    {
        ...
        async run(bot, msg) {
            const guild = await bot.finder.guild(msg.args.length > 0 ? msg.args.join(' ') : msg.guild.id);
            return await msg.embed({
                ...
                fields: [
                    ...
                    {
                        name: `Emojis [${guild.emojis.length}]`, value: (await bot.finder.emojis(guild, 25)), inline: true
                    }
                ],
    ...
    },
    {
        ...
        async run(bot, msg) {
            ...
            if (msg.guild && msg.guild.members.has(user.id)) {
                ...
                const mutual = bot.guilds.filter((i) => i.members.has(user.id)).map(g => g.name).join(', ');
                if (member.nick)

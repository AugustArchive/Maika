        ...
        },
        {
            ...
            async run(msg) {
                ...
                if (intervals) {
                    ...
                    await msg.bot.r.table('intervals').insert({ id: msg.sender.id, daily: Date.now() }).run();
            ...
            run: async(msg) => {
                if (msg.args[0]) {
                    if (msg.args[0].toLowerCase() === 'guild') {
                        let leaderboard = await msg.bot.r.table('users').orderBy(msg.bot.r.desc('coins')).run();
                        ...
                        const longestName = Math.max(...leaderboard.map(balance => (msg.bot.users.has(balance.id) ? `${msg.bot.users.get(balance.id).username}#${msg.bot.users.get(balance.id).discriminator}` : 'Unknown#0000').length));
                        return msg.code('fix', leaderboard.map((balance, i) => (i + 1) + '.' + Array(6 - ((i + 1) + '.').length).join(' ') + (msg.bot.users.has(balance.id) ? msg.bot.users.get(balance.id).username + '#' + msg.bot.users.get(balance.id).discriminator : 'Unknown') + Array((longestName + 3) - (msg.bot.users.has(balance.id) ? msg.bot.users.get(balance.id).username + '#' + msg.bot.users.get(balance.id).discriminator : 'Unknown').length).join(' ') + ' | ' + balance.coins.toLocaleString()).join('\n'));
                    } else if (msg.args[0].toLowerCase() === 'global') {
                        const lb = await msg.bot.r.table('users').orderBy(msg.bot.r.desc('coins')).limit(20).run();
                        const longestName = Math.max(...lb.map(balance => (msg.bot.users.has(balance.id) ? `${msg.bot.users.get(balance.id).username}#${msg.bot.users.get(balance.id).discriminator}` : 'Unknown#0000').length));
                        return msg.code('fix', lb.map((balance, i) => (i + 1) + '.' + Array(6 - ((i + 1) + '.').length).join(' ') + (msg.bot.users.has(balance.id) ? msg.bot.users.get(balance.id).username + '#' + msg.bot.users.get(balance.id).discriminator : 'Unknown') + Array((longestName + 3) - (msg.bot.users.has(balance.id) ? msg.bot.users.get(balance.id).username + '#' + msg.bot.users.get(balance.id).discriminator : 'Unknown').length).join(' ') + ' | ' + balance.coins.toLocaleString()).join('\n'));
                    }
                } else {
                        ...
                        case "set": {
                            ...
                            switch (msg.args[1]) {
                                case "description": {
                                    ...
                                    await msg.bot.r.table('users').get(msg.sender.id).update({ profile: { description: desc } }).run();
                                    msg.reply(`**${msg.sender.username}**: Your description has been set to: **\`${desc}\`**`);
                                } break;
                                case "social.osu": {
                                    await msg.bot.r.table('users').get(msg.sender.id).update({ profile: { social: { osu: msg.args[2] } } }).run();
                                    msg.reply(`**${msg.sender.username}**: Your osu!profile has been set to: <https://osu.ppy.sh/u/${msg.args[2]}>`);
                                } break;
                                case "social.reddit": {
                                    await msg.bot.r.table('users').get(msg.sender.id).update({ profile: { social: { reddit: msg.args[2] } } }).run();
                                    msg.reply(`**${msg.sender.username}**: Your reddit profile has been set to: <https://reddit.com/u/${msg.args[2]}>`);
                                } break;
                                case "social.steam": {
                                    await msg.bot.r.table('users').get(msg.sender.id).update({ profile: { social: { steam: msg.args[2] } } }).run();
                                    msg.reply(`**${msg.sender.username}**: Your reddit profile has been set to: ${msg.args[2]}`);
                                } break;
                                case "social.twitter": {
                                    await msg.bot.r.table('users').get(msg.sender.id).update({ profile: { social: { twitter: msg.args[2] } } }).run();
